...
from train.augmenters import GameSegment, MovementSegment
...
class ConvRes1d(nn.Module):
    ...
    def __init__(
        ...
        self.lns = nn.ModuleList(
            [nn.LayerNorm([in_channels, chunk_length])]
            + [nn.LayerNorm([out_channels, chunk_length]) for _ in range(n_convs)]
            + [nn.LayerNorm([out_channels, chunk_length])]
        )
...
class MlpGSVAE(nn.Module, InvarMixin1):
    ...
    def setup(self, output_segment: MovementSegment):
        self.input_rms.update(
            self.pack_invar(output_segment.three_p).reshape(-1, self.input_size)
        )
    ...
    def forward(
        ...
        z = self.encode(x, w)
        zz = z.reshape(
            z.shape[0], 1, self.sentence_length, self.vocab_size
...
class CondMlpGSVAE(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, w: torch.Tensor = None, n: int = None
    ) -> (torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor):
        z = self.encode(x, w)
...
class CondTransformerGSVAE(nn.Module, InvarMixin1):
    ...
    def setup(
        ...
        self.threep_rms.update(
            self.pack_invar(history[:, None]).reshape(-1, self.threep_size)
        )
    ...
    def encode_game(
        ...
        game_obj_embeds, game_obj_mask = self.embed_game_obj(
            game_notes, game_bombs, game_obstacles
        )
    ...
    def predict_logits(
        ...
        game_obj_embeds, game_obj_mask, game_hist_embeds, game_hist_mask = (
            self.encode_game(game_notes, game_bombs, game_obstacles, game_history)
        )
    ...
    def encode_style(
        ...
        # print("encode_style")
        playstyle_obj_embeds, playstyle_obj_mask = self.embed_game_obj(
            playstyle_notes, playstyle_bombs, playstyle_obstacles
        )
        playstyle_hist_embeds, playstyle_hist_mask = self.embed_3p(
            self.pack_invar(playstyle_history)
        )
        playstyle_3p_embeds, playstyle_3p_mask = self.embed_3p(
            self.pack_invar(playstyle_3p)
        )
        ...
        for i, embeds in enumerate(domain_encode_me):
            ...
            div_term = (
                torch.arange(0, de.shape[-1] // 2, 1, device=de.device).float()
                / (de.shape[-1] // 2)
                * 2
                * np.pi
            )
        ...
        emb_catted = torch.cat(
            [emb_catted, torch.zeros_like(emb_catted[:, :, [0]])], dim=2
        )
        mask_catted = torch.cat(
            [mask_catted, torch.zeros_like(mask_catted[:, :, [0]])], dim=2
        )

        # Encode the playstyle references independently
        playstyle_tokens = self.playstyle_encoder.forward(
            src=emb_catted.flatten(0, 1), src_key_padding_mask=mask_catted.flatten(0, 1)
        )[..., -1, :].unflatten(0, (emb_catted.shape[0], emb_catted.shape[1]))
    ...
    def predict_logits_from_embeds(
        ...
        embedded_seq = self.logit_predictor.forward(
            src=xxww, src_key_padding_mask=mask_simple
        )
    ...
    def embed_game_obj(
        self, notes: torch.Tensor, bombs: torch.Tensor, obstacles: torch.Tensor
    ) -> (torch.Tensor, torch.Tensor):
        embeddings = []
        masks = []
    ...
    def embed_3p(self, my_3p: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        ...
        ww = self.threep_proj(
            self.threep_rms.normalize(my_3p).unflatten(-1, (-1, self.effective_3p_size))
        )
        ...
        pe[..., 0::2] = torch.sin(
            torch.arange(ww.shape[-2], device=ww.device)[..., None] * div_term
        )
        pe[..., 1::2] = torch.cos(
            torch.arange(ww.shape[-2], device=ww.device)[..., None] * div_term
        )
    ...
    def sample_from_z(
        ...
        if topk == 1:
            assert torch.all(
                hard.argmax(-1) == topk_indices.squeeze(-1)
            ), "topk=1 should match argmax"
...
class GameplayEncoder(nn.Module, InvarMixin1):
    ...
    def encode(
        ...
        embedded_seq = self.tenc.forward(
            src=xxww.flatten(0, 1), src_key_padding_mask=mask_simple.flatten(0, 1)
        ).unflatten(0, (xxww.shape[0], xxww.shape[1]))
        z = embedded_seq[..., -1, :]
        return z
...
class ConvGSVAE(nn.Module, InvarMixin):
    ...
    def __init__(
        ...
        self.encoder_proj = nn.Linear(
            self.input_size * self.chunk_length, self.hidden_size * self.sentence_length
        )
        self.encoder = nn.Sequential(
            *(
                nn.Sequential(
        ...
        self.decoder_deproj = nn.Linear(
            self.hidden_size * self.sentence_length, self.input_size * self.chunk_length
        )
...
class TransformerGSVAE(nn.Module, InvarMixin):
    ...
    def __init__(
        ...
        self.encoder_fc = nn.Sequential(
            nn.Linear(
                self.embed_size * self.effective_chunk_length * 3, self.hidden_size
            ),
            nn.LeakyReLU(),
        ...
        self.decoder_fc = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(
                self.hidden_size, self.embed_size * self.effective_chunk_length * 3
            ),
        )
        self.deproj = nn.Sequential(
            nn.Linear(self.embed_size, self.hidden_size),
    ...
    def setup(self, my_3p: torch.Tensor):
        self.input_rms.update(
            self.pack_invar(my_3p[:, None]).reshape(-1, self.effective_input_size)
        )
    ...
    def encode(self, x: torch.Tensor, w: torch.Tensor = None) -> torch.Tensor:
        ...
        div_term = torch.exp(
            torch.arange(0, z.shape[-1] // 2, 1).float()
            * (-np.log(1000.0) / (z.shape[-1]))
        ).to(z.device)
        pe[..., 0::2] = torch.sin(idxs[..., None] * div_term[None])
        pe[..., 1::2] = torch.cos(idxs[..., None] * div_term[None])
        z = z + pe
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        div_term = torch.exp(
            torch.arange(0, zz.shape[-1] // 2, 1).float()
            * (-np.log(1000.0) / (zz.shape[-1]))
        ).to(zz.device)
        pe[..., 0::2] = torch.sin(idxs[..., None] * div_term[None])
        pe[..., 1::2] = torch.cos(idxs[..., None] * div_term[None])
        zz = zz + pe
    ...
    def forward(
        ...
        z = self.encode(x, w)
        zz = z.repeat_interleave(n, 1).unflatten(
            -1, (self.sentence_length, self.vocab_size)
        )
        soft, hard = gumbel_softmax(zz, tau=self.tau)
        k = hard.argmax(-1)
...
class CondTransformerDenoiser(nn.Module):
    ...
    def __init__(
        ...
        self.temb = nn.Sequential(
            nn.Linear(1, hidden_size), nn.SiLU(), nn.Linear(hidden_size, embed_size)
        )
    ...
    def forward(
        ...
        div_term = torch.exp(
            torch.arange(0, xwt.shape[-1] // 2, 1).float()
            * (-np.log(1000.0) / (xwt.shape[-1]))
        ).to(xwt.device)
        pe[..., 0::2] = torch.sin(idxs[..., None] * div_term[None])
        pe[..., 1::2] = torch.cos(idxs[..., None] * div_term[None])
        xwt += pe
    ...
    def embed_history(self, history: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        ...
        # ww = self.history_proj(self.history_rms.normalize(history))
        ww = self.history_proj(
            self.history_rms.normalize(history).reshape(
                history.shape[0], -1, 3, self.effective_history_size
            )
        )
        ...
        div_term = torch.exp(
            torch.arange(0, ww.shape[-1] // 2, 1).float()
            * (-np.log(10000.0) / (ww.shape[-1]))
        ).to(ww.device)
        de[..., 0::2] = torch.sin(3 * div_term)
        de[..., 1::2] = torch.cos(3 * div_term)
        ww += de

        w_mask = torch.isnan(t_orig.reshape(t_orig.shape[0], -1)).repeat_interleave(
            3, -1
        )
...
class ConditionalEDM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, w: torch.Tensor):
        ...
        if self.rolling_yes:
            yes = (
                torch.rand(x.shape[0]) < 0.4
            )  # Make sure to change this for rolling diffusion...
        ...
        eps_tar = x
        eps_out = c_skip * noised_x + c_out * self.denoise(
            c_in * noised_x, w, c_noise
        ).view(x.shape)
    ...
    def step(self, x_t: torch.Tensor, w: torch.Tensor, tt: int):
        ...
        eps_out = c_skip * x_t + c_out * self.denoise(c_in * x_t, w, c_noise).view(
            x_t.shape
        )
