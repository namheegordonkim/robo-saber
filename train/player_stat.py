    ...
    for shard_idx, arrow_file, shard_size in zip(shard_idxs, arrow_files, shard_sizes):
        ...
        for idx in range(shard_size):
            try:
                ...
                player_d = xror.data["info"]["user"]
                players.append(player_d)
                # total_ranked_score = xror.data["info"]["user"]["totalRankedScore"]
                # ranked_play_count = xror.data["info"]["user"]["rankedPlayCount"]
                # with np.errstate(divide='ignore', invalid='ignore'):
                #     normalized_ranked_score = total_ranked_score / ranked_play_count
                #     normalized_ranked_score = np.nan_to_num(normalized_ranked_score)
                # total_ranked_scores.append(total_ranked_score)
                # ranked_play_counts.append(ranked_play_count)
                # normalized_ranked_scores.append(normalized_ranked_score)
                # player_d = {
                #     "total_ranked_score": total_ranked_score,
                #     "ranked_play_count": ranked_play_count,
                #     "normalized_ranked_score": normalized_ranked_score,
                # }
                # if idx > 10:
                #     break
        ...
        df.to_csv(f"{proj_dir}/out/{args.run_name}/{args.out_name}/players_{job_array_i:03d}.csv", index=False)
        # np.savetxt(f"{outdir}/scores_{job_array_i:03d}.txt", np.array(total_ranked_scores), fmt="%d")
