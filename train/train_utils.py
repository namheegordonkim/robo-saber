...
from beaty_common.bsmg_xror_utils import open_xror, open_bsmg, quat_to_expm, open_unpacked_xror, open_beatmap_from_bsmg_or_boxrr
...
placeholder_3p = np.loadtxt(f"{proj_dir}/data/beaterson/placeholder_3p.txt", dtype=np.float32)
placeholder_3p_sixd = np.loadtxt(f"{proj_dir}/data/beaterson/placeholder_3p_sixd.txt", dtype=np.float32)
...
def collect_rollout(
    ...
    with maybe_no_grad(not train_yes):
        ...
        recon_loss = torch.nn.functional.mse_loss(y_ref, game_3p[:, None, None], reduction="none").mean((-1, -2, -3))
        ...
        if matching_loss_type == "jsd":
            ...
            matching_loss = torch.nn.functional.mse_loss(z_hard, z_hard_ref)
...
class BoxrrHFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for index_pair in index_pair_set:
            ...
            opened = open_unpacked_xror(XROR.unpack(xror_raw["xror"]), True)
            ret.append(opened)
...
def load_shards(q: Queue, sem: Semaphore, shard_names: list[str], init_i: int):
    # Perpetually iterate through the shards, pushing to the throttled queue
    while True:
        for shard_name in shard_names[init_i:]:
            sem.acquire()
            d = torch.load(shard_name)
            q.put(d)
            # break  # same shard over and over


class BoxrrCacheDataset(IterableDataset):
    def __init__(self, data_dir, batch_size, init_i):
        self.data_dir = data_dir
        self.shard_names = sorted(glob.glob(f"{data_dir}/**/part_*.pkl", recursive=True))
        self.current_shard_i = -1
        self.current_shard = self.shard_names[0]
        # self.open_next_shard()
        self.sample_i = init_i
        self.batch_size = batch_size
        self.q = Queue(maxsize=5)
        self.sem = Semaphore(5)
        self.proc = Thread(target=load_shards, args=(self.q, self.sem, self.shard_names, init_i))
    ...
    def open_next_shard(self):
        ...
        self.d = torch.load(self.current_shard)
...
def nanpad_collate_fn(batch):
    ...
    for k in list(batch[0].keys()):
        ...
        if isinstance(batch[0][k], np.ndarray) or isinstance(batch[0][k], torch.Tensor):
            ...
            max_seq_len = torch.clip(max_seq_len, min=40)
...
def xror_to_tensor_collate_fn(batch):
    return nanpad_collate_fn([open_unpacked_xror(XROR.unpack(x["xror"]), True) for x in batch])
