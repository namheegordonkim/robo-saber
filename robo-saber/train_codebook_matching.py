...
import pauli
from beaty_common.bsmg_xror_utils import open_unpacked_xror
...
def main(args):
    ...
    umds_df = pd.read_csv(f"{proj_dir}/data/boxrr_umds_v14.csv")
    ...
    R_df = pd.read_csv(f"{proj_dir}/data/R_v13.csv")
    ...
    num_workers = 0
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=nanpad_collate_fn, pin_memory=True, num_workers=num_workers, shuffle=True)
